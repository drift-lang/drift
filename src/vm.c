...
void free_frame(frame *f) { printf("free GC\n"); }
...
builtin bts[BUILTIN_COUNT] = {{"println", BU_FUNCTION, bt_println},
        ...
        if (v->item == i && fn->value.fn.mutiple != NULL) {
          ...
          if (arg->item == 0) {
            ...
            while (arg->item > 0) {
              ...
              a->value.arr.element = append_keg(a->value.arr.element, p);
            }
          }
          obj = a;
        } else {
...
void reg_name(char *name, object *obj) { add_table(TOP_TB, name, obj); }

void push_stack(object *obj) { PUSH(obj); }
