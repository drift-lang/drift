...
void free_tokens(keg *g) {
        for (int i = 0; i < g->item; i++) {
                ...
                if (tok->kind == FLOAT || tok->kind == NUMBER ||
                    tok->kind == CHAR || tok->kind >= 39) {
                        free(tok->literal);
                }
                free(tok);
        }
...
#define GET_PR_OBJ() \
        (object *)top_code()->objects->data[( \
            *(int16_t *)top_code()->offsets->data[vst.op - 1])]
...
void type_error(type *T, object *obj) {
        fprintf(stderr,
                "\033[1;31mvm %d:\033[0m expect type %s, but found %s.\n",
...
void ob_type(frame *f, keg *arg) {
        ...
        str->kind = OBJ_STRING;
        str->value.string =
            (char *)obj_type_string((object *)pop_back_keg(arg));
        PUSH(str);
}
...
void check_interface(object *in, object *cl) {
        ...
        for (int i = 0; i < elem->item; i++) {
                ...
                if (!type_eq(m->ret, fn->value.fn.ret)) {
                        simple_error(
                            "return type in the method are inconsistent");
                }
                ...
                for (int j = 0; j < m->T->item; j++) {
                        ...
                        if (!type_eq(a, b)) {
                                simple_error(
                                    "Inconsistent types of method arguments");
                        }
                }
        }
}
        ...
        while (vst.ip < top_code()->codes->item) {
                ...
                case STORE_NAME: {
                        ...
                        if (T->kind == T_USER && obj->kind == OBJ_CLASS) {
                                ...
                                if (ptr == NULL) {
                                        simple_error("cannot store to "
                                                     "undefined interface");
                                }
                                object *in = (object *)ptr;
                                if (in->kind != OBJ_INTERFACE) {
                                        simple_error(
                                            "type is not interface or class");
                                }
                ...
                case ASSIGN_TO: {
                        ...
                        if (origin->kind == OBJ_INTERFACE) {
                                if (obj->kind != OBJ_CLASS) {
                                        simple_error("interface needs to be "
                                                     "assigned by whole");
                                }
                ...
                case TO_OR: {
                        ...
                        if (a->kind == OBJ_STRING && b->kind == OBJ_STRING) {
                                ...
                                if (la + lb > STRING_CAP) {
                                        simple_error(
                                            "number of characters is greater "
                                            "than 128-bit bytes");
                                }
                        }
                ...
                case TO_INDEX: {
                        ...
                        if (obj->kind == OBJ_ARR) {
                                ...
                                if (p->value.integer >=
                                    obj->value.arr.element->item) {
                                        simple_error("index out of bounds");
                                }
                                PUSH((object *)obj->value.arr.element
                                         ->data[p->value.integer]);
                        }
                        if (obj->kind == OBJ_MAP) {
                                ...
                                for (; i < obj->value.map.k->item; i++) {
                                        if (obj_eq(p, (object *)obj->value.map
                                                          .k->data[i])) {
                                                PUSH((object *)obj->value.map.v
                                                         ->data[i]);
                                                break;
                                        }
                                }
                        ...
                        if (obj->kind == OBJ_STRING) {
                                ...
                                ch->kind = OBJ_CHAR;
                                ch->value.ch =
                                    obj->value.string[p->value.integer];
                                PUSH(ch);
                        }
                        break;
                }
                case TO_REPLACE: {
                        ...
                        if (j->kind == OBJ_ARR) {
                                ...
                                if (j->value.arr.element->item == 0) {
                                        ...
                                        insert_keg(j->value.arr.element, 0,
                                                   obj);
                                } else {

                                        if (p >
                                            j->value.arr.element->item - 1) {
                                                simple_error(
                                                    "index out of bounds");
                                        }
                        ...
                        if (j->kind == OBJ_MAP) {
                                ...
                                for (int i = 0; i < j->value.map.k->item; i++) {
                                        if (obj_eq(idx, (object *)j->value.map
                                                            .k->data[i])) {
                                                p = i;
                                                break;
                                        }
                ...
                case TO_REP_SUR: {
                        ...
                        if (j->kind == OBJ_ARR) {
                                ...
                                void *r = binary_op(
                                    op, (object *)j->value.arr.element->data[p],
                                    obj);
                                if (r == NULL) {

                                        unsupport_operand_error(
                                            code_string[code]);
                                }
                                replace_keg(j->value.arr.element, p,
                                            (object *)r);
                        ...
                        if (j->kind == OBJ_MAP) {
                                ...
                                object *r = binary_op(
                                    op, (object *)j->value.map.v->data[p], obj);
                                if (r == NULL) {
                                        unsupport_operand_error(
                                            code_string[code]);
                                }
                ...
                case CALL_FUNC: {
                        ...
                        if (fn->value.fn.std) {
                                for (int i = 0;
                                     i < sizeof(bts) / sizeof(bts[0]); i++) {
                                        if (strcmp(bts[i].name,
                                                   fn->value.fn.name) == 0) {
                                                for (int i = 0, j = arg->item;
                                                     i < k->item; i++) {
                                                        type *T =
                                                            (type *)v->data[i];
                                                        object *obj =
                        ...
                        frame *f =
                            new_frame(fn->value.fn.code); /* Func code object */

                        for (int i = 0; i < k->item; i++) {
                                char *N = (char *)k->data[i];
                                type *T = (type *)v->data[i]; /* Type */
                                object *p =
                                    arg->data[--arg->item]; /* Reverse */
                ...
                case GET_OF: {
                        ...
                        if (obj->kind != OBJ_ENUMERATE &&
                            obj->kind != OBJ_CLASS && obj->kind != OBJ_TUP &&
                            obj->kind != OBJ_INTERFACE &&
                            obj->kind != OBJ_MODULE) {
                                simple_error("only enum, face, module, tuple "
                                             "and whole type "
                                             "are supported");
                        }
                        if (obj->kind == OBJ_ENUMERATE) {
                                        ...
                                        if (strcmp(m->name, name) == 0) {
                                                object *wh =
                                                    (object *)
                                                        obj->value.in.class;
                                                frame *fr =
                                                    (frame *)wh->value.cl.fr;
                                                vst.whole = wh;
                                                PUSH((object *)get_table(fr->tb,
                                                                         name));
                                                goto next;
                                        }
                                }
                        ...
                        if (obj->kind == OBJ_MODULE) {
                                void *p =
                                    get_table((table *)obj->value.mod.tb, name);
                                if (p == NULL) {
                                        undefined_error(name);
                                }
                                PUSH((object *)p);
                ...
                case SET_OF: {
                        ...
                        if (obj->kind != OBJ_CLASS && obj->kind != OBJ_MODULE) {
                                simple_error("only members of class and module "
                                             "can be set");
                        }
                        table *tb = NULL;
                        if (obj->kind == OBJ_CLASS) {
                        ...
                        if (!basic(j)) {
                                simple_error("only members of basic objects "
                                             "can be assign");
                        }
                ...
                case NEW_OBJ: {
                        ...
                        if (obj->kind != OBJ_CLASS) {
                                simple_error(
                                    "only whole object can be created");
                        }
                        ...
                        new->value.cl.fr =
                            (struct frame *)new_frame(obj->value.cl.code);
                ...
                default: {
                        fprintf(stderr,
                                "\033[1;31mvm %d:\033[0m unreachable '%s'.\n",
...
keg *read_path(keg *pl, char *path) {
        ...
        if ((dir = opendir(path)) == NULL) {
                printf("%s\n", path);
                simple_error(
                    "failed to open the std library or current directory");
        }
...
void load_module(char *name) {
        ...
        if (!ok) {
                fprintf(stderr,
                        "\033[1;31mvm %d:\033[0m undefined module '%s'.\n",
                        GET_LINE(), name);
                exit(EXIT_FAILURE);
        } else {
                free_keg(pl);
