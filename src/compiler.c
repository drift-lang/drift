...
u_int8_t get_set_ass_code(token_kind kind) {
    if (kind == AS_ADD) return SE_ASS_ADD; if (kind == AS_SUB) return SE_ASS_SUB;
    if (kind == AS_MUL) return SE_ASS_MUL; if (kind == AS_DIV) return SE_ASS_DIV;
    return SE_ASS_SUR;
}
