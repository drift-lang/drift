...
object *binary_op(u_int8_t op, object *a, object *b) {
    ...
    case TO_ADD:
        OP_A(a, b, je, +)
        if (a->kind == OBJ_STRING &&
            b->kind == OBJ_STRING) { /* string + string */
            je->kind = OBJ_STRING;
    ...
    case TO_DIV:
        ...
        if (a->kind == OBJ_FLOAT) {
            ...
            if (b->kind == OBJ_FLOAT &&
                b->value.floating > 0) { /* float / float */
...
bool type_checker(type *tp, object *obj) {
    ...
    default: {
        if ((tp->kind == T_INT && obj->kind != OBJ_INT) ||
            ...
            if (tp->kind == T_USER) {
                const char *name =
                    tp->inner.name; /* Often deal with 'any' type here */
